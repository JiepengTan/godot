---
name: TileMap 导出插件
overview: 实现 GDScript 编辑器插件，将 Godot 编辑器中的 TileMap 场景导出为 SPX 运行时加载所需的紧凑 JSON 格式，并自动复制纹理文件到导出目录。
todos:
  - id: plugin-config
    content: 创建 plugin.cfg 和主插件类 SpxTileMapExporter
    status: pending
  - id: extractor-layer
    content: 实现 TileMapLayer 数据提取（含 Base64 编码）
    status: pending
    dependencies:
      - plugin-config
  - id: extractor-tileset
    content: 实现 TileSet 和 TileSetAtlasSource 数据提取
    status: pending
    dependencies:
      - plugin-config
  - id: extractor-physics
    content: 实现碰撞多边形提取（flat 数组格式）
    status: pending
    dependencies:
      - extractor-tileset
  - id: json-export
    content: 实现 JSON 序列化和文件写入
    status: pending
    dependencies:
      - extractor-layer
      - extractor-physics
  - id: texture-copy
    content: 实现纹理文件复制到导出目录
    status: pending
    dependencies:
      - extractor-tileset
  - id: menu-integration
    content: 集成到 Godot 编辑器 Tools 菜单
    status: pending
    dependencies:
      - json-export
      - texture-copy
