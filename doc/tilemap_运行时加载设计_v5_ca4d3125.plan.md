---
name: TileMap 运行时加载设计
overview: 设计 SPX TileMap 运行时加载机制，使用单一紧凑 JSON 格式描述 TileSet 和 TileMap 数据，通过 SpxResMgr 直接加载图片资源，无需 Godot 引擎的 import 流程。配套提供 Godot 编辑器插件用于导出。
todos:
  - id: design-json-format
    content: 设计并确认紧凑的单一 JSON 格式规范
    status: completed
  - id: impl-tileset-parser
    content: 实现 TileSet JSON 解析和 TileSet 对象创建
    status: completed
    dependencies:
      - design-json-format
  - id: impl-tilemap-parser
    content: 实现 TileMap JSON 解析和 TileMapLayer 创建（含 Base64 解码）
    status: completed
    dependencies:
      - impl-tileset-parser
  - id: impl-physics-collision
    content: 实现物理碰撞多边形解析和设置
    status: completed
    dependencies:
      - impl-tileset-parser
  - id: impl-cache-system
    content: 实现 TileSet 和 Texture 缓存机制
    status: completed
    dependencies:
      - impl-tileset-parser
  - id: impl-api
    content: 实现外部调用 API 和 GDExtension 绑定
    status: completed
    dependencies:
      - impl-tilemap-parser
  - id: design-export-plugin
    content: 设计 Godot 编辑器导出插件架构（暂不实现）
    status: completed
---
...
| [spx_tilemap_parser_mgr.h](modules/spx/spx_tilemap_parser_mgr.h) | Godot 对象创建方法声明 |

| [spx_tilemap_parser_mgr.cpp](modules/spx/spx_tilemap_parser_mgr.cpp) | 实现 Godot 对象创建（使用数据结构） |
