...
def configure_msvc(env: "SConsEnvironment", vcvars_msvc_config):
    ...
    if env["silence_msvc"] and not env.GetOption("clean"):
        ...
        def spawn_capture(sh, escape, cmd, args, env):
            ...
            try:
                # Try multiple encodings for MSVC output (GBK for Chinese systems, fallback to UTF-8)
                try:
                    with open(tmp_stdout_name, "r", encoding="gbk", errors="replace") as tmp_stdout:
                        lines = tmp_stdout.read().splitlines()
                except (UnicodeDecodeError, OSError):
                    try:
                        with open(tmp_stdout_name, "r", encoding="utf-8", errors="replace") as tmp_stdout:
                            lines = tmp_stdout.read().splitlines()
                    except (UnicodeDecodeError, OSError):
