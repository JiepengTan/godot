...
const GodotAudioRecorder = {
	...
	$GodotAudioRecorder: {
		...
		findGameCanvas: function() {
			// Method 1: Via global variable (like windows.gameCanvas mentioned by the user)
			if (typeof window !== 'undefined' && window.gameCanvas) {
				return window.gameCanvas;
			}
			
			// Method 2: Find Godot's Canvas (usually the first Canvas or the one with id 'canvas')
			let canvas = document.getElementById('canvas');
			if (canvas && canvas.tagName === 'CANVAS') {
				return canvas;
			}
			
			// Method 3: Find the first Canvas element
			const canvases = document.getElementsByTagName('canvas');
			if (canvases.length > 0) {
				return canvases[0];
			}
			
			// Method 4: Find via Module object (common way in Emscripten)
